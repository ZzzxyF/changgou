...
public class OrderController {
    ...
    public Result<Order> add(@RequestBody(required = false)   Order order){
        if(ObjectUtils.isEmpty(order)){
            order=new Order();
        }
        String username= tokenDecode.getUserInfo().get("username");
        order.setUsername(username);
        Order orderTemp=orderService.add(order);
        return new Result<Order>(true,StatusCode.OK,"创建订单成功",orderTemp);
    }
